...
use PrestaShop\PrestaShop\Core\Localization\DateTimeFormatterInterface;
...
class AdminDbBlogPostController extends ModuleAdminController
{
    ...
    public function renderForm()
    {
        ...
        /**
         * Aquí formateamos la fecha de publicación programada en el formulario
         */
        if ($obj->scheduled_date != null) {
            $obj->scheduled_date = date('Y-m-d', strtotime($obj->scheduled_date));
        ...
        $this->fields_form = array(
            ...
            'input' => array(
                array(
                    ...
                    'name' => 'scheduled_date',
                    'required' => false,
                    'lang' => false, // No es necesario traducir el campo
                    'desc' => $this->l('Selecciona la fecha deseada de publicación para este artículo'),
                    'hint' => $this->trans('Formato: yyyy-mm-dd', array(), 'Admin.Global'),
                ),
                array(
                    'type' => 'hidden',
    ...
    public function processAdd()
    {
        ...
        if ($object && $object->id > 0) {
            //6. Aquí procesamos la fecha de publicación programada en processAdd
            if (Tools::getValue('scheduled_date') !== null) {
                $scheduled_date = DateTime::createFromFormat('Y-m-d', Tools::getValue('scheduled_date'));
                $object->scheduled_date = $scheduled_date->format('Y-m-d');
            } else {
    ...
    public function processUpdate()
    {
        ...
        if ($object != false && $object->id_dbblog_post > 0) {
            //7. Aquí volvermos a procesar la fecha de publicación programada en processUpdate
            if (Tools::getValue('scheduled_date') !== null) {
                $scheduled_date = DateTime::createFromFormat('Y-m-d', Tools::getValue('scheduled_date'));
                $object->scheduled_date = $scheduled_date->format('Y-m-d');
            } else {
